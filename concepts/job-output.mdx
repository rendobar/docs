...
One output file. `file` is that file, `files` lists one, `data` is null. The most common pattern for transform jobs (FFmpeg, transcode, watermark, render).
...
- **Transform jobs** (`ffmpeg`, `transcode`, `watermark.*`, `caption.burn`, `render.*`) return files with `data: null`.
