...
`output` is present when `status` is `complete`. A `failed` job carries an `error` object instead. See [Job lifecycle](/concepts/job) for status transitions.
...
- [Job lifecycle](/concepts/job): the statuses a job moves through before `output` exists
