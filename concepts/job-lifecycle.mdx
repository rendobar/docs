...
The dashboard connects to a per-organization WebSocket hub (OrgHub) for real-time job status updates. Events include job creation, status changes, progress, and balance updates. The connection uses Cloudflare Durable Objects with hibernation for zero-cost idle connections.

For programmatic use, configure [webhooks](/guides/webhooks) to receive push-based HTTP notifications when jobs complete or fail.
