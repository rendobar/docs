...
sidebarTitle: "Job"
description: "Run a job from a typed request, through async execution on our workers, to a signed output. The lifecycle, the statuses, and the output shape, in one place."
icon: "gear"
...
sequenceDiagram
    ...
    API->>Worker: dispatch with signed URLs
    Worker->>Worker: download, run, upload
    Worker-->>API: result callback
...
| `dispatched` | Sent to a worker with presigned URLs for download and upload. |
| `running` | The worker is downloading inputs, running the work, and uploading the result. |
