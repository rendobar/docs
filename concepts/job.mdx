...
To also write the output to a bucket you own, add `destinations`. An input can come from that bucket too, as a `storage://` URI:

```json
{ "type": "ffmpeg", "inputs": { "source": "storage://demo-bucket/raw/clip.mp4" }, "params": { "command": "ffmpeg -i source out.mp4" }, "destinations": ["storage://demo-bucket/exports"] }
```
