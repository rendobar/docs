...
sidebarTitle: "Job"
description: "Run a job from a typed request, through async execution on a provider, to a signed output. The lifecycle, the statuses, and the output shape, in one place."
icon: "gear"
...
sequenceDiagram
    ...
    API->>Provider: dispatch with signed URLs
    Provider->>Provider: download, run, upload
    Provider-->>API: result callback
...
On any terminal status the job is billed for the compute it used, and a [webhook](/guides/webhooks) fires if you configured one. Failures carry an `error.code` from the [error catalogue](/support/errors). A job that runs past its type's time limit fails with `PROVIDER_TIMEOUT` and is not charged.
