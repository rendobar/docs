...
| Storage | `storage:read` | `storage:write` | Connect, change and remove buckets |
...
Storage scopes were added on 2026-09-13. Keys created and apps connected before then do not hold
them, so the [storage](/storage) endpoints and `client.storage` answer them with this `403`. Create a
new key, or reconnect the app and allow Storage. A job that reads from or delivers to a connected
bucket needs only `jobs:write`, so existing keys still run those.
