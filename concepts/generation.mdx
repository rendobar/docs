...
`GET /models` returns the live catalog: every model with its supported jobs, tier, capabilities, step range, and an estimated per-image cost. Filter to one job type with `?job=`. Build a model picker against this endpoint instead of hardcoding the list.
...
The `estCostUsd` field is an estimate to help you compare models, not a fixed price. Jobs bill on the compute they use, cost-plus. See [credits and billing](/concepts/credits).
