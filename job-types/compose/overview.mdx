---
title: "Compose: render from a timeline"
sidebarTitle: "Overview"
description: "Render a video from a declarative JSON timeline. Tracks, clips, transitions, and per-clip effects, in one API call."
icon: "film"
tag: "Beta"
keywords: ["video compose api", "json timeline to video", "video transitions api", "compose"]
canonical: "https://rendobar.com/docs/job-types/compose/overview"
---

<script
  type="application/ld+json"
  dangerouslySetInnerHTML={{
    __html: JSON.stringify({
      "@context": "https://schema.org",
      "@type": "TechArticle",
      "@id": "https://rendobar.com/docs/job-types/compose/overview/#article",
      "headline": "Compose: render from a timeline",
      "description": "Render a video from a declarative JSON timeline. Tracks, clips, transitions, and per-clip effects, in one API call.",
      "datePublished": "2026-06-21",
      "dateModified": "2026-06-21",
      "author": { "@type": "Organization", "@id": "https://rendobar.com/#organization" },
      "publisher": { "@type": "Organization", "@id": "https://rendobar.com/#organization" },
      "isPartOf": { "@id": "https://rendobar.com/#website" }
    })
  }}
/>

<Note>
**Beta.** Compose is rolling out. Parameters below are stable, the full field reference lands with general availability.
</Note>

`compose` renders a video from a JSON timeline. Start with one clip, then layer up: more clips, transitions, overlays, and effects. This page builds from the smallest render to the full model. Each step adds one idea.

## Your simplest render

The smallest timeline is one track with one clip. This trims three seconds of a video and renders it to MP4. The whole API call:

<CodeGroup>

```ts SDK
import { createClient, outputUrl } from "@rendobar/sdk";

const client = createClient({ apiKey: "rb_YOUR_KEY" });

const job = await client.jobs.create({
  type: "compose",
  inputs: {},
  params: {
    schemaVersion: 1,
    output: { format: "mp4", resolution: { width: 1280, height: 720 }, fps: 30 },
    timeline: {
      tracks: [
        { clips: [
          { asset: { type: "video", src: "https://cdn.rendobar.com/assets/examples/a.mp4", trim: { from: 0, to: 3 } }, start: 0, length: 3 },
        ] },
      ],
    },
  },
});

const result = await client.jobs.wait(job.id);
console.log(outputUrl(result));
```

```bash cURL
curl -X POST https://api.rendobar.com/jobs \
  -H "Authorization: Bearer rb_YOUR_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "type": "compose",
    "inputs": {},
    "params": {
      "schemaVersion": 1,
      "output": { "format": "mp4", "resolution": { "width": 1280, "height": 720 }, "fps": 30 },
      "timeline": {
        "tracks": [
          { "clips": [
            { "asset": { "type": "video", "src": "https://cdn.rendobar.com/assets/examples/a.mp4", "trim": { "from": 0, "to": 3 } }, "start": 0, "length": 3 }
          ] }
        ]
      }
    }
  }'
```

</CodeGroup>

<Info>
**Assets:** video, image, text, audio · **Output:** mp4 / mov / webp / jpg / png · **Timeout:** 600 s
</Info>

That is the shape of every render. Everything below is one more clip, one more track, or one more field.

## Join two clips

Put a second clip on the same track and a transition between them. The clips overlap by the transition's `duration`, so the second clip starts a little before the first ends.

```json
"clips": [
  { "asset": { "type": "video", "src": "a.mp4", "trim": { "from": 0, "to": 3 } }, "start": 0, "length": 3 },
  { "type": "transition", "transition": "crossfade", "duration": 1 },
  { "asset": { "type": "video", "src": "b.mp4", "trim": { "from": 0, "to": 3 } }, "start": 2, "length": 3 }
]
```

Crossfade, fade, slide, and zoom are on the [Transitions](/job-types/compose/transitions) page.

## How the timeline works

Now the model, in two ideas:

- **Tracks stack.** The first track is the background. Each later track composites on top of the ones below it: overlays, titles, picture-in-picture.
- **Clips sit in time.** Each clip has a `start` and a `length` in seconds. Two clips on the same track are joined by a transition. A gap between them plays nothing.

So a multi-track timeline is layers (tracks) of sequences (clips). That is the whole structure.

## Add a layer

A second track renders over the first. This lays an animated title over the video.

```json
"tracks": [
  { "clips": [ { "asset": { "type": "video", "src": "a.mp4" }, "start": 0, "length": 5 } ] },
  { "clips": [ {
      "asset": { "type": "text", "text": "Coastal Escape",
        "style": { "font": "Inter", "weight": 800, "size": 84, "color": "#FFFFFF",
                   "position": { "x": "50%", "y": "40%" }, "animate": { "type": "slideUp", "duration": 0.7 } } },
      "start": 0, "length": 5 } ] }
]
```

Titles, lower-thirds, and chroma key are on [Compositing and text](/job-types/compose/compositing-text).

## Style a clip

Any clip takes effects as fields. Add `color` to grade it, `speed` to slow it down, `transform` to animate it, `chromaKey` to key out a background. One field at a time, on the clip it applies to.

```json
{ "asset": { "type": "video", "src": "a.mp4" }, "start": 0, "length": 5,
  "color": { "contrast": 1.12, "saturation": 1.2, "temperature": 6800 } }
```

Each effect, with a before and after, lives on [Motion and speed](/job-types/compose/motion), [Color and filters](/job-types/compose/color), and [Compositing and text](/job-types/compose/compositing-text).

## Set the canvas

`output` controls the resolution, frame rate, and format. A `9:16` canvas with a `crop` on the clip turns landscape footage into a vertical Reel that fills the frame, no black bars.

```json
"output": { "format": "mp4", "resolution": { "width": 1080, "height": 1920 }, "fps": 30 }
```

<Frame caption="Landscape footage reframed to a vertical 9:16 Reel (crop)">
  <video src="https://cdn.rendobar.com/assets/showcase/fx/reframe.mp4" autoPlay loop muted playsInline style={{ maxWidth: "300px" }} />
</Frame>

## Go deeper

<CardGroup cols={2}>
  <Card title="Transitions" icon="arrow-right-arrow-left" href="/job-types/compose/transitions">
    Crossfade, fade, slide, and zoom between two clips.
  </Card>
  <Card title="Motion & speed" icon="wand-magic-sparkles" href="/job-types/compose/motion">
    Transform and Ken Burns, picture-in-picture, slow-motion, timelapse.
  </Card>
  <Card title="Color & filters" icon="palette" href="/job-types/compose/color">
    Cinematic grades, lifting flat footage, dramatic black and white.
  </Card>
  <Card title="Compositing & text" icon="layer-group" href="/job-types/compose/compositing-text">
    Chroma key, blend modes, opacity fades, blurred backdrops, titles.
  </Card>
</CardGroup>
