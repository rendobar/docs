...
Use `storage://` URIs to read from and write to a [connected bucket](/storage):

```ts
await client.jobs.create({
  type: "ffmpeg",
  inputs: { source: "storage://demo-bucket/raw/clip.mp4" },
  params: { command: "ffmpeg -i source out.mp4" },
  destinations: ["storage://demo-bucket/exports"],
});
```

`jobs.get(id)` returns `deliveries`, one entry per destination, and `jobs.retryDeliveries(id)` retries the failed ones. `client.storage` manages the connections. See [SDK methods](/storage#sdk-methods).
