...
<Frame caption="The node details panel before install. The verified badge means n8n reviewed the code and publishes the version itself, and Install node makes the node available to everyone on the instance.">
  <img src="/images/n8n/install-detail.png" alt="The n8n node details panel for Rendobar before install. The Rendobar icon and a verified badge sit next to the name, an orange Install node button sits to the right, and below them are the description, the Verified label, the download count and the author." />
</Frame>
