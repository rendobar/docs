...
curl -X POST https://api.rendobar.com/jobs \
  ...
  -d '{
    ...
    "params": {
      "redactTypes": ["faces", "license_plates"],
      "method": "blur"
    }
  }'
```
...
|-----------|------|---------|-------------|
| `redactTypes` | string[] | `["faces", "license_plates"]` | What to redact. Choose any of `faces`, `license_plates`, `text`. |
| `method` | string | `"blur"` | Redaction style: `blur` (gaussian), `pixelate`, or `blackbox` (solid fill) |
| `blurStrength` | integer | `30` | Blur radius, only used when `method: "blur"` (1--100) |
| `threshold` | number | `0.5` | Detection confidence threshold (0--1) |
| `padding` | integer | `10` | Padding around detected regions in pixels (0--100) |
