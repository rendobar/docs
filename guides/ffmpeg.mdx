...
Every completed job, FFmpeg or otherwise, returns the same `output` shape:

- `output.data`: the computed answer (a probe result, a transcript). `null` for jobs that only write files.
- `output.file`: the headline result you play or download, one file or a stream manifest. `null` for data-only jobs and pure file sets.
- `output.files`: every file the job produced, the complete list. `[]` for data-only jobs.
