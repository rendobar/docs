...
The `inputs` map is the working directory for your command. Each key is a filename that gets staged under that exact name. Each value is the source for that file. Your command references the file by its bare name.
...
| `{ "ref": "uploads/..." }` | A file you already uploaded |
...
```json
{
  ...
  "inputs": {
    ...
    "logo.png": { "ref": "uploads/brand-logo.png" }
  },
  "params": {
...
Inline `-i https://...` URLs in the command still work, so `inputs` can stay `{}` for simple jobs. Use `inputs` when a file needs a stable name or has no public URL.
