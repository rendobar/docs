...
6. **Return**: signed `output.url` available via `GET /jobs/{id}`
...
A completed job carries an `output` object discriminated on `kind`. A command that writes one file returns `output.kind: "file"` with the signed `url` and probed `meta`:
...
A command that writes many files (HLS or DASH segments, image sequences, `-f segment`) returns `output.kind: "stream"` or `output.kind: "set"` instead. It carries the full set:
...
For a stream, `output.url` is the playable manifest. Point a player at it directly. An unordered collection (image sequence, resolution ladder) has `output.kind: "set"` and no top-level `url`. Read `files` or `manifestUrl` for the full list.
