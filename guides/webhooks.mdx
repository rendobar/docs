...
The `output` shape is identical for every job type. A data-only job (such as `extract.metadata`) carries its answer in `output.data` with `file` null and `files` empty. A stream job carries the manifest as `output.file` and every segment in `output.files`. See [Job output](/concepts/job-output) for each pattern.

For `job.failed`, the payload carries `error` instead of `output`. The error matches the [GET /jobs](/concepts/job-output) error shape: `code`, `message`, `detail` (the provider stderr tail, or null), and `retryable`.
...
- [Job output](/concepts/job-output): the canonical `output` and `error` shape this payload carries
