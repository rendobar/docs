...
| `job.delivery_succeeded` | The output was written to one [storage destination](/storage#delivery-events). Carries `storageId`, `path`, and `url` |
| `job.delivery_failed` | Writing the output to one storage destination failed. Carries `storageId` and the [`reason`](/storage#delivery-errors) |
| `job.deliveries_settled` | Every storage destination of the job has resolved. Sent once, with every outcome |
