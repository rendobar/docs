...
A job with `destinations` also sends `job.deliveries_settled` to `callback.url` once every destination has resolved. It is the only [delivery event](/storage#delivery-events) a callback receives.
