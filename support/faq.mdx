...
<Accordion title="Can I run custom FFmpeg commands?">
  Yes. That is the entire `ffmpeg` job type. Commands run through 8 security layers (flag whitelist, format/filter blocklists, sandbox isolation) before execution. [FFmpeg](/job-types/ffmpeg).
</Accordion>
