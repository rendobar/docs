...
- **Signed download URL** (`output.url`) is valid for **1 hour**. Re-fetch via `GET /jobs/{id}` for a fresh URL.
- **Output retention**: 7 days (Free) or 30 days (Pro). After that, R2 deletes the file; the job record stays but `output.url` returns 404.
