...
| `PROVIDER_ERROR` | 502 | Processing failed on the worker. Check `error.message` for details |
| `PROVIDER_TIMEOUT` | 504 | The job did not finish in time. No credits charged |
...
- [Job lifecycle](/concepts/job): where `PROVIDER_ERROR` and `PROVIDER_TIMEOUT` surface
