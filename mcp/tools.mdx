...
- `complete`: adds `cost` and `output`. For a single file, `output: { kind: "file", url, expiresAt, poster, meta: { format, width, height, durationMs, sizeBytes } }`. HLS/DASH returns `kind: "stream"`, image sequences return `kind: "set"`
