...
<ParamField body="type" type="string" required>
  Job type from the registry. `ffmpeg` is the only Live type today.
</ParamField>

<ParamField body="inputs" type="Record<string, string>" required>
  Map of input name to URL. For `ffmpeg`, keys match filenames in the command.
</ParamField>

<ParamField body="params" type="Record<string, unknown>">
  Type-specific parameters. For `ffmpeg`: `{ command: "ffmpeg -i input -c copy output.mp4" }`.
</ParamField>
...
- `complete`: adds `cost` and `output`. The `output` shape is the same for every job type: `{ data, file, files, expiresAt }`. Read `output.file.url` for the file to play or download, `output.data` for a computed answer. See [Job output](/concepts/job-output)
- `failed`: adds `error: { code, message, detail, retryable }`
...
**Returns:** `{ jobs: [{ id, type, status, createdAt, cost, output? }], total }`. `output` is included only when `status` is `complete`. Read `output.file.url` for the file, or `output.data` for a computed answer.
...
- [FFmpeg reference](/job-types/ffmpeg): the main job type called by `submit_job`
