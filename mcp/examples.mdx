...
When `get_job` returns `status: "complete"`, it includes `output.file.url`. The agent shows you a clickable link. Open it in your browser and the captioned video downloads.
...
- [`ffmpeg` job type](/jobs/ffmpeg): what FFmpeg commands are accepted
...
- [FFmpeg guide](/jobs/ffmpeg): the FFmpeg security model the agent runs against
