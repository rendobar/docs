...
The music bed above, drawn out. Track 1 is the background and the two clips follow each other because neither names a `start`. The music names `length: "end"`, so it stops on the last frame whatever the video adds up to.

```mermaid
gantt
    title Two clips and a music bed, in seconds
    dateFormat X
    axisFormat %S
    section Track 2, music
    audio, length "end" : 0, 12
    section Track 1, video
    river.mp4            : 0, 6
    forest.mp4           : 6, 12
```
...
A `composition` asset holds a whole timeline of its own, so a group of layers can be moved, scaled and faded together rather than one at a time.
