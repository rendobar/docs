...
A `shape` asset draws a rect, ellipse, circle, triangle or diamond. No image to
make, no file to host, and it stays sharp at any output resolution.
...
you set `anchor`, so a full-width bar sits at `x: "50%"` whatever its width. And
there is no `length`, because a shape has no duration of its own: like text and
images it runs to the end of its container unless you give it one.
...
```json
{ "type": "shape", "color": "#0A0A0ACC", "radius": 0.35,
  "position": { "x": "26%", "y": "50%" },
...
`stroke` draws a border just inside the shape's edge. Give the shape a `color`
too and you get a filled shape with a border, and `radius` rounds both together.
...
`width` is in pixels of the output frame, so it stays the same visual weight
whatever the shape's size.
...
a spotlight or a vignette rather than a hard-edged disc.

### Animation

`animateTo` moves, resizes and turns the shape over the clip. `easing` is
`linear` by default, which is what a progress bar or a timer wants, or `smooth`
for an entrance.
...
A shape that is square in pixels stays square as it grows, so an animated circle
does not turn into an ellipse halfway through.
...
| `radius` or a two-colour fill plus stroke, with `rotate` or `softness` | the primitive that rounds and carries two colours cannot turn or feather | drop one, or rotate the whole clip with `transform.rotate` |
| a fill and a stroke on a non-`rect` | that primitive carries one colour | put the fill on its own shape clip below |
| an animated `rotate` beyond -180 to 180 | the engine turns a shape over exactly one revolution | animate `transform.rotate` for a full spin |

### What else a shape takes

Every per-clip field applies. `transform` repositions and scales it, `blendMode`
mixes it with the layer below, `blur` softens the whole thing, and opacity
...
What a shape is not is a drawing surface. There is no gradient fill and no drop
shadow. Anything past a filled or outlined rect, ellipse, circle, triangle or
diamond is still an `image` asset.
...
| `text` | `text`, `style` |
| `shape` | `shape` (`rect`, `ellipse`, `circle`, `triangle`, `diamond`), `color` (fill, omit for outline-only), `stroke` `{ color, width }`, `position` (the **centre** by default), `anchor`, `size` `{ width, height? }` (omit the height for a shape square in pixels), `radius`, `rotate`, `softness`, `animateTo` `{ position, size, rotate, easing }`. See [Shapes](#shapes) |
