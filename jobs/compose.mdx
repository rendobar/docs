...
A `shape` asset draws a rectangle. No image to make, no file to host, and it
stays sharp at any output resolution.
...
Two things to know up front. `position` is the **centre** of the shape, not a
corner, so a full-width bar sits at `x: "50%"` whatever its width. And there is
no `length`, because a shape has no duration of its own: like text and images it
runs to the end of its container unless you give it one.

`width` is a percent of the frame's width, `height` a percent of its height.

### Transparency goes in the colour

`color` takes `#RRGGBB` or the 8-digit `#RRGGBBAA`. Use the alpha byte for a
constant translucency, the way `#0A0A0ACC` does above.
...
`radius` runs from `0` (square) to `1` (fully round).

At `1`, a wide box becomes a pill and a **pixel-square** box becomes a circle.
Square in pixels, not in percent: on a 16:9 frame a width of `10%` needs a
height of `17.8%` (that is `10 × 16/9`). On a 9:16 frame the same width needs
`5.6%`.

```json
{ "type": "shape", "color": "#059669", "position": { "x": "12%", "y": "80%" },
  "size": { "width": "10%", "height": "17.8%" }, "radius": 1 }
```

### Outlines

`stroke` draws a border just inside the shape's edge. Set `color` to
`#00000000` and the fill disappears, leaving an outline-only highlight box.

```json
{ "type": "shape", "color": "#00000000",
  "position": { "x": "72%", "y": "30%" },
  "size": { "width": "36%", "height": "34%" },
  "stroke": { "color": "#F59E0B", "width": 4 } }
```

`stroke` cannot be combined with `radius` or with `animateTo`, and the API
rejects both combinations rather than rendering them wrong. The outline is drawn
as a square band, so a rounded shape would end up with square corners inside a
rounded edge, and it cannot follow a shape that moves.

### A progress bar is a shape that grows

`animateTo` moves and resizes the shape over the clip, in a straight line.
Starting from a width of `0%` gives you a progress bar.

```json
{ "type": "shape", "color": "#FFFFFF",
  "position": { "x": "50%", "y": "97%" },
  "size": { "width": "0%", "height": "1.5%" },
  "animateTo": { "size": { "width": "96%" } } }
```

A dimension you leave out of `animateTo` keeps its starting value, so the bar
above grows sideways without changing height. `transform` cannot do this: its
`scale` is uniform, so it would grow the bar in both directions at once.

### Lines and letterbox bars

There is no `line` type, because a line is a rectangle with a small height.

```json
{ "type": "shape", "color": "#FFFFFF", "position": { "x": "50%", "y": "50%" },
  "size": { "width": "60%", "height": "0.4%" } }
```

For a diagonal, centre the shape and add `transform` to the clip:
`"transform": { "rotate": 30 }`. Rotation turns the frame about its centre, so a
centred shape spins in place and an off-centre one swings around the middle of
the frame.

Letterbox bars are two shapes: one at `y: "5%"`, one at `y: "95%"`, both
`height: "10%"` and `color: "#000000"`. A full-frame scrim is one shape at
`100%` by `100%` with a colour like `#00000080`.

### What else a shape takes

Every per-clip field applies. `transform` rotates and repositions it,
`blendMode` mixes it with the layer below, `blur` softens its edges, and opacity
keyframes fade it in and out. A track holding only shapes composites as a base
layer, so a solid colour background is a timeline that references no files at
all.

What a shape is not is a drawing surface. There is no gradient fill, no ellipse
and no drop shadow. A rounded rectangle, a pill, a pixel-square circle and an
outline are the whole set. Anything past that is still an `image` asset.
...
| `text` | `text`, `style` |
| `shape` | `color`, `position` (the shape's **centre**), `size` `{ width, height }`, `radius` (`0` to `1`), `stroke` `{ color, width }`, `animateTo` `{ position, size }`. See [Shapes](#shapes) |
