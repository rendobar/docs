...
<ParamField body="timeout" type="integer">
  Max execution time in seconds. Defaults to your plan's full job budget (Free 1 hour, Pro 9 hours), so you only set this when you want a job to fail sooner. Asking for more than your plan allows is rejected at submit with `PLAN_LIMIT`.
</ParamField>
...
Plan job budgets: 1 hour (Free), 9 hours (Pro). That is a bound on processing time, not on how long your output can be. See [plan limits](/support/limits) for the full table. Exceeding the timeout returns `RUNNER_TIMEOUT`, and no credits are charged.
