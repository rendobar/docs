...
5. Rendobar writes the whole output to your bucket with the connection's credential, as described in [what gets delivered](#what-gets-delivered). The result is reported as a [delivery event](#delivery-events).
...
A destination receives the job's whole `output`, whatever the job returns. The output path splits into a name and an extension, and every piece lands under that name:

| The job returns | Written to your bucket, for the path `exports/clip.mp4` |
|---|---|
| One file | `exports/clip.mp4` |
| Several files, such as an HLS ladder or image frames | The folder `exports/clip/`, each file under its own name and subfolder |
| Structured data only, such as `ffprobe` | `exports/clip.json` |
| A file and structured data, such as `captions.animate` | `exports/clip.mp4` and `exports/clip.json` |
| Several files and structured data | The folder `exports/clip/` and `exports/clip.json` |

If the file is itself `.json`, its data is written to `exports/clip.data.json` instead.

There is no size limit on a delivery beyond your bucket's own. Large files and long file sets are written in parts over as many attempts as they need, and the delivery stays `pending` until the last part lands.

With **Keep both**, Rendobar checks the output name and the `.json` name before writing. If either is taken, the job's suffix is added to the name, so every piece moves together, for example to `exports/clip-4a1b6e/`. Inside a folder, only its first file is checked, so add `{job_id}` to the output path if files from different jobs must never share a folder.
...
| `path` | The key written, after the template and any rename. A folder ends in `/` |
| `url` | The public URL if one is set, otherwise a Rendobar URL. Absent for a folder |
| `reason` | Why the delivery failed. Present only when `status` is `failed` |
| `renamed` | `true` when **Keep both** added a suffix |

For a single file, `output.file.url` and `deliveries[].url` point to the same content. The first is a Rendobar URL that expires at `output.expiresAt`. The second points to the copy in your bucket, which Rendobar does not delete. For a folder, each file's key is the folder `path` plus that file's `output.files[].path`.
...
| `not_deliverable` | The job failed, was cancelled, stored no output, or its output expired before a retry | Run the job again if you still need its output delivered |
...
Listing and reading connections needs the `storage:read` scope. Connecting, testing, changing and
deleting them needs `storage:write` and the owner or admin role. See [scopes](/concepts/scopes).
