...
sequenceDiagram
    ...
    Rendobar->>Runner: Dispatch the job with a signed URL for the input
    Runner->>Bucket: Read the input through the signed URL
    Runner->>Runner: Run the job
    Runner->>Rendobar: Upload the output to Rendobar storage
    Rendobar->>Bucket: Write the output with the connection's credential
```

The signed URL covers one object and expires, so the runner never holds your credential. The output is stored on Rendobar first, then written to your bucket. Nothing in the bucket is made public.
...
sequenceDiagram
    ...
    par Each destination is written independently
