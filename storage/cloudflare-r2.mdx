...
Rendobar runs its [access checks](/storage#access-checks) and saves the connection only if they pass. If the bucket does not exist, the dialog offers **Create the bucket and connect**.
...
**Egress.** R2 does not charge for egress. See [R2 pricing](https://developers.cloudflare.com/r2/pricing/).
