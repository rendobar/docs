...
The binary is called `rb`. It auto-uploads local inputs, submits an [`ffmpeg`](/jobs/ffmpeg) job, and downloads the result to the path your `ffmpeg` command names.
...
The CLI does one thing: run FFmpeg jobs. It does not bundle filter chaining, batch primitives, or the other [job types](/jobs/ffmpeg). Submit those via the [SDK](https://www.npmjs.com/package/@rendobar/sdk) or [`POST /jobs`](/jobs/ffmpeg).
...
- [FFmpeg reference](/jobs/ffmpeg): the API the CLI calls under the hood
